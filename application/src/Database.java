...
public class Database {
    ...
    private void loadDatabase() {
        ...
        if(!databaseFile.exists()) {
            try {
                if(!databaseFile.getParentFile().mkdirs() && !databaseFile.createNewFile()) {
                    System.out.println("Не удалось создать файл базы данных для хранения животинок из питомничка");
                    return;
                } else {
                    System.out.println("Файл базы данных не обнаружен, создан новый");
                }
            } catch (Exception e) {
                System.out.println(e.getMessage());
                return;
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (data.length >= 3) {
                    ...
                    switch (className) {
                        ...
                        case "Hamster" -> animal = new Humster(name, skills);
